...
Only this docstring note was added. The fork's prior version of this file
(158 lines) was a subset of this one (249 lines); the added surface is
mostly the `typed_browser_state` / `typed_browser_prepare` /
`typed_browser_action` default methods (each returns a graceful
"unavailable" response for any backend — including this fork's stubbed
`tools/computer_use/browser_route.py` path — that doesn't implement typed
browser routing), not anything macOS-specific being removed.
