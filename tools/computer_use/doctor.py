...
prior `doctor.py`. Only this docstring note was added; every other line
is unmodified upstream. Builds on gap #1's PR #152, which vendored the
cross-platform `tools/computer_use/{__init__,backend,cua_backend,schema,
tool,vision_routing}.py` this module's `_cua_child_env()` /
`cua_driver_child_env()` calls depend on.
"""
