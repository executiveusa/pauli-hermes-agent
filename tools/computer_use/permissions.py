...
openspec/changes/hermes-upstream-gap-map). New file — the fork's prior
macOS-only computer_use package had no separate cross-platform permission
module; permission handling lived inline in the old SkyLight-SPI backend
that gap #1 already replaced. Only this docstring note was added; every
other line is unmodified upstream. Per the gap-map's explicit guidance
("Keep upstream platform permission checks; bind Pauli consequential-action
approvals above them"), this module is left exactly as upstream wrote it —
it only answers "is the OS-level driver ready to be controlled at all"
(TCC grants / driver health), which is orthogonal to and sits below
Hermes's own consequential-action approval gate
(``tools/computer_use_tool.py::set_approval_callback`` /
``cli.py::_computer_use_approval_callback``, unchanged by this PR): a
command still needs Pauli approval to run even when this module reports
the platform is fully permitted.
"""
