...
Only this docstring note was added.

The fork's prior version of this file (749 lines) gated
`check_computer_use_requirements()` to `sys.platform == "darwin"` only —
non-macOS hosts got `False` unconditionally (see the now-removed
`test_check_fn_is_false_on_linux` in tests/tools/test_computer_use.py,
which this vendor's test file no longer contains). This version's
`check_computer_use_requirements()` (below) allows "darwin", "win32", and
"linux" alike, gated only on whether the `cua-driver` binary is actually
available — the real substance of gap #1.

The fork's `set_approval_callback`/`_approval_callback` module-global hook
that `cli.py::_computer_use_approval_callback` wires up is present in this
upstream version unchanged (same function, same global, same docstring
referencing the `terminal_tool._approval_callback` pattern) — verified by
diffing top-level def names between the fork's prior file and this one
before vendoring, so no fork-specific approval wiring was lost.
