...
openspec/changes/hermes-upstream-gap-map). Only this docstring note was
added; every other line is unmodified upstream.

This replaces the minimal fork-native inert stub gap #1's PR #152 landed
at this same path (to satisfy `cua_backend.py`'s module-load-time import
of `CuaTypedBrowserRoute` without pulling in the full typed-browser
feature). That stub's every entry point (`observe`/`prepare`/`mutate`)
returned a `"typed_browser_unavailable"` refusal; this real implementation
restores the full session-scoped typed `cua_browser_*` adapter (exact
pid/window_id binding, semantic-ref capability tracking, trust-route
choice between `trusted` and `dom_event` input, existing-profile grant
enforcement) upstream's `cua_backend.py` was written against. No other
vendored file needed to change: `CuaTypedBrowserRoute.__init__`'s
signature (`session_id`, `call_tool`, `has_tool`) and `BrowserRouteState`'s
public shape are identical between the stub and this real version, so
`cua_backend.py`'s `_browser_route()` / `_handle_transport_reset()`
callers from gap #1 work unmodified.
"""
