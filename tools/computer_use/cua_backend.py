...
This note is the only addition; every other line is upstream's.

The fork's prior version of this file (702 lines) was macOS-only, driving
`cua-driver` purely as a background SkyLight-SPI controller. This version
(3955 lines, ~5.6x) is the reason the whole gap exists: the bulk of the
growth is genuine cross-platform breadth (Linux X11 window discovery incl.
`xprop`-based active-window resolution, WSL path translation
(`_wsl_windows_path_to_posix`), Windows null-pid/placeholder-window-id
handling, an embedded-daemon fallback (`_EmbeddedCuaDaemon`) for hosts
without a system-installed driver, driver version/update-nudge plumbing,
