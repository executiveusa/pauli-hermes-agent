...
(tools/spill_safety.py) as part of the hooks/observability gap-map port
(gap #6, "Hooks + observability") — a transitive dependency of
tools/hook_output_spill.py discovered during recon (not one of the 7
files originally flagged as missing, but required to vendor
hook_output_spill.py correctly rather than reimplementing its
symlink-safe write path). This file did not exist in this fork before
this port; the fork's own spill-file handling
(tools/tool_result_storage.py) does not do the O_EXCL / symlink-refusing
write this module provides, so nothing here overlaps or is replaced.
