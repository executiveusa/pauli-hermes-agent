...
When browser.backend is "browser-use", the model gets a ``browser_exec`` tool
that drives the Browser Use CLI 3.0. It coexists with (does not replace) the
classic browser_* toolset in tools/browser_tool.py and its siblings — which
backend a given session is offered is a runtime/config decision
(``browser.backend`` — "off" always keeps the classic tools, "browser-use"
always prefers this one, "" auto-detects whether the CLI is runnable), not a
code-level replacement.

Vendored verbatim from NousResearch/hermes-agent @ v2026.8.16.2
(tools/browser_use_cli.py) as part of the browser_exec gap-map port
(gap #3, "browser_exec / newer browser routing"). This file did not exist
in this fork before this port; it is new, additive code alongside the
existing browser tooling, not a replacement for it.
"""
