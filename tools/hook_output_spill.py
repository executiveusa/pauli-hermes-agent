...
(tools/hook_output_spill.py) as part of the hooks/observability gap-map
port (gap #6, "Hooks + observability"). This file did not exist in this
fork before this port. Wired additively into agent/shell_hooks.py's
production hook-firing path (_make_callback) only — NOT into the
`hermes hooks test` / `hermes hooks doctor` CLI diagnostic paths, where
seeing the full untruncated output is the point.

Originally ported upstream from openai/codex PR #21069 (``Spill large
hook outputs from context``).
