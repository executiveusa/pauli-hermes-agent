...
v2026.8.16.2 (commit-dated 2026-08-22), part of the priority-1 upstream
gap-map item "coding posture" (openspec/changes/hermes-upstream-gap-map).
Only this docstring note was added — no functional changes.

Wiring status in this fork: only the default ``auto`` (prompt-only) path
is connected — see the coding-posture block in
``agent/system_prompt.py::build_system_prompt_parts``. The opt-in
``focus`` mode's toolset-collapse (``RuntimeMode.toolset_selection`` /
``coding_selection``) and skill-index compaction
(``RuntimeMode.compact_skill_categories`` / ``coding_compact_skill_categories``)
are present and correct but not called from anywhere in this fork yet —
deliberately deferred as separate follow-up work, since wiring those means
touching this fork's CLI/TUI toolset-dispatch and skill-index-rendering
code paths, which is materially higher-risk than the read-only prompt
addition landed here. Both remain safe to leave unwired: nothing calls
them, so they're inert until a follow-up PR does.
