...
(agent/outbound_webhooks.py) as part of the hooks/observability gap-map
port (gap #6, "Hooks + observability"). This file did not exist in this
fork before this port; it is new, additive code registered onto the
fork's existing plugin hook manager (hermes_cli.plugins.get_plugin_manager),
not a replacement for anything. It is the outbound mirror of the fork's
existing inbound webhook platform (gateway/platforms/webhook.py).
