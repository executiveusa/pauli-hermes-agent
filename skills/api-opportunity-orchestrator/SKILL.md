...
description: Discover, validate, specify, route, and verify monetizable API/connector opportunities. Hermes orchestrates; Pauli's Place workers build. Use for free/public API scans, paid wrapper APIs, MCP/CLI connector products, Composio extensions, workflow APIs, or API-key businesses.
---
...
## Trigger
Use when the owner asks to find free/public APIs, package data or capabilities, create custom integrations, build an API/MCP/CLI product, use Composio for customer connectors, turn a portal workflow into an agent-callable endpoint, or find small API businesses the digital workers can execute.

## Operating loop
1. **Inventory before invention.** Search owned repos, Pauli's Place integrations, Composio, Zatanna-style workflow API options, and existing APIs/CLIs/MCPs before proposing new code.
...
5. **Choose the thinnest connector form.** Prefer reuse in this order: existing native capability -> official API -> Composio toolkit/session -> Composio extension/proxy -> existing OpenAPI/GraphQL/MCP via `mcp2cli` -> authorized workflow API layer such as Zatanna when the needed action is trapped behind a portal/UI -> small custom wrapper -> browser/computer automation fallback -> new underlying service only if the value cannot be obtained otherwise.
...
## Workflow API rule
Use a workflow-API vendor/layer when all are true:
- the customer is authorized to use the target portal/system;
- the needed workflow is missing or materially incomplete in the official API;
- the workflow is frequent or valuable enough that browser automation maintenance is economically unattractive;
- third-party terms, customer permissions, and applicable law permit the intended automation;
- the commercial offer does not depend on reselling the vendor's service contrary to its license/terms.

Zatanna is a candidate infrastructure vendor, not assumed resale inventory. Its current public terms grant service use for internal business purposes and place responsibility on the customer for rights to connected systems. Any reseller/white-label use requires separate written permission or an appropriate commercial agreement before it becomes part of a customer offer.
...
- Authorized workflow API architecture for portals without adequate official APIs, including session/auth lifecycle, source change detection, retries, and evidence.
...
## Human gates
Stop for: money movement, contracts/paid vendor commitments, credential/access changes, production launch, outbound campaigns, legal/licensing claims, regulated/high-stakes customer claims, third-party portal automation whose permission is not established, or destructive changes.
...
## Stop conditions
Reject or park when upstream terms prohibit the intended use/resale, the source is unstable without a defensible normalization moat, no audience-matched paid analog exists and no buyer behavior can be tested cheaply, the product depends on unsupported legal/compliance claims, portal automation lacks authorization, or engineering begins before a customer/price/test is defined.
