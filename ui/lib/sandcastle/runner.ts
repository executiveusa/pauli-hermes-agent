...
export async function getRuns(): Promise<SandcastleRun[]> {
  return listSandcastleRuns();
}

export async function getRun(runId: string): Promise<SandcastleRun | undefined> {
  return getSandcastleRun(runId);
}

export async function getRunEvents(runId: string): Promise<SandcastleEvent[] | undefined> {
  const run = getSandcastleRun(runId);
  if (!run) return undefined;
  return getSandcastleEvents(runId);
}

export async function stopRun(runId: string): Promise<boolean> {
  const run = getSandcastleRun(runId);
  if (!run) return false;
  await stopSandcastleRun(runId);
  return true;
}

export async function retryRun(runId: string): Promise<SandcastleRun> {
  const result = await retrySandcastleRun(runId);
  return result.run;
}

export async function requestApproval(runId: string): Promise<boolean> {
  const run = getSandcastleRun(runId);
  if (!run) return false;
...
export async function approveMerge(runId: string, notes?: string): Promise<boolean> {
  const run = getSandcastleRun(runId);
  if (!run) return false;
  run.approvalStatus = 'approved';
  run.status = 'completed';
  emitEvent(runId, 'merge_approved', `Merge approved${notes ? ': ' + notes : ''}`);
  return true;
}

export async function discardRun(runId: string, reason?: string): Promise<boolean> {
  const run = getSandcastleRun(runId);
  if (!run) return false;
  await discardSandcastleRun(runId);
  if (reason) {
    emitEvent(runId, 'sandbox_discarded', `Discarded: ${reason}`, {}, 'info');
  }
  return true;
}
