...
Vendored from NousResearch/hermes-agent @ v2026.8.16.2
(tests/tools/test_browser_use_cli.py) as part of the browser_exec gap-map
port (gap #3).
...
class TestToolSurfaceSwap:
    ...
    def test_browser_exec_in_browser_toolsets(self):
        ...
        assert "browser_exec" in TOOLSETS["hermes-acp"]["tools"]
        assert "browser_exec" in TOOLSETS["hermes-api-server"]["tools"]
        # Upstream also asserts a "coding" posture toolset here (the
        # coding-posture work — gap-map item #5 — lands separately as its
        # own PR and isn't in this fork's toolsets.py yet); skip that
        # assertion rather than assert against a toolset that doesn't exist
        # on this branch.
...
class TestLegacyCloudMigration:
    ...
    @pytest.mark.skip(
        reason=(
            "Upstream's dedicated 'Browser Use CLI' picker row "
            "(TOOL_CATEGORIES['browser']['providers'] entry with "
            "browser_backend=...) and its _is_provider_active() special case "
            "are a tools-config UX addition left for a follow-up PR — out of "
            "scope for this port, which wires browser_exec itself. "
            "browser.backend: browser-use is fully functional set directly "
            "in config.yaml."
        )
    )
...
class TestProviderPickerIntegration:
    ...
    @pytest.mark.skip(
        reason=(
            "Upstream's dedicated 'Browser Use CLI' picker row is a "
            "tools-config UX addition left for a follow-up PR — see "
            "TestLegacyCloudMigration.test_picker_highlights_cli_row_for_migrated_config."
        )
    )
    ...
    @pytest.mark.skip(reason="Requires the not-yet-ported 'Browser Use CLI' picker row.")
...
@pytest.mark.skip(
    reason=(
        "The `/browser use [off]` slash command lives in upstream's "
        "hermes_cli/cli_commands_mixin.py, which does not exist on this "
        "fork (its slash-command dispatch is structured differently) — "
        "porting it is out of scope for this PR, which wires browser_exec "
        "itself. browser.backend: browser-use/off is fully functional set "
        "directly in config.yaml; left as a follow-up CLI UX PR."
    )
)
