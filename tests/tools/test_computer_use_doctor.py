...
prior doctor test coverage. Originally at ``tests/computer_use/
test_doctor.py`` upstream; moved to ``tests/tools/`` and renamed
``test_computer_use_doctor.py`` to match this fork's existing
`tests/tools/test_computer_use*.py` convention (gap #1's PR #152 did not
introduce a separate ``tests/computer_use/`` directory). Only this note
and the file's location/name were changed; test bodies are unmodified
upstream.
"""
