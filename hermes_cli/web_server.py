...
async def serve_vault_graph(path: str, request: Request):
    """Serves the static visual knowledge graph generated by Graphify."""
    # Support token in query parameter for iframe loads, fallback to headers
    ...
    from pathlib import Path
    vault_path = os.environ.get("OBSIDIAN_VAULT_PATH", "E:\\OBSIDIAN SECOND BRAIN")
    graphify_dir = Path(vault_path) / "graphify-out"
    
    if not graphify_dir.exists():
        raise HTTPException(status_code=404, detail="Graphify index has not been built yet.")
        
    target = (graphify_dir / path).resolve()
    # Security: check that the resolved path is inside the graphify-out directory
    if not str(target).startswith(str(graphify_dir.resolve())):
        raise HTTPException(status_code=403, detail="Forbidden")
        
    if target.is_dir() or not path:
        target = graphify_dir / "index.html"
